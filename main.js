...
io.on('connection', (socket) => {
    ...
    socket.on('check_update', () => {
        ...
        const versionUrl = 'https://YOUR_USERNAME.github.io/YOUR_REPO_NAME/version.json';
    ...
    socket.on('apply_update', (zipUrl) => {
        console.log('[SYSTEM] Downloading and applying update...');
        // 1. Download the zip
        // 2. Unzip it (GitHub puts everything in a folder like 'gameZONE-main')
        // 3. Copy the contents out of that folder into the current directory
        // 4. Delete the leftover zip and empty folder
        const updateCommand = `curl -L -o update.zip "${zipUrl}" && unzip -q -o update.zip && cp -a *-main/* . && rm -rf *-main update.zip`;
        ...
        exec(updateCommand, { cwd: __dirname }, (err, stdout, stderr) => {
            if (err) {
                ...
                // Give the UI 3 seconds to show the success message, then reboot Linux
